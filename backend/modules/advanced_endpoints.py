...
# Imports will be done inside functions to avoid circular imports with main.py

router = APIRouter()

# Multi-Agent System Endpoints

@router.post("/api/multi-agent/process")
async def process_with_agents(data: dict):
    """Process request using multi-agent system"""
    try:
        from app.main import get_multi_agent_system
...
@router.get("/api/multi-agent/capabilities")
...
@router.post("/api/testing/generate-tests")
async def generate_tests(data: dict):
    """Generate tests for code"""
    try:
        from app.main import get_ai_testing_suite
...
@router.post("/api/testing/analyze-coverage")
...
@router.post("/api/testing/visual-regression")
...
@router.post("/api/knowledge-graph/add-member")
async def add_team_member(data: dict):
    """Add team member to knowledge graph"""
    try:
        from app.main import get_knowledge_graph
...
@router.get("/api/knowledge-graph/find-expert/{topic}")
...
@router.get("/api/knowledge-graph/recommendations/{person_id}")
...
async def get_analytics_dashboard(days: int = 30):
    ...
    try:
        from app.main import get_predictive_analytics
...
@router.post("/api/analytics/project-forecast")
...
@router.post("/api/analytics/team-insights")
...
async def scan_file_security(data: dict):
    ...
    try:
        from app.main import get_security_copilot
...
@router.post("/api/security/check-compliance")
...
@router.post("/api/performance/analyze-file")
async def analyze_performance(data: dict):
    """Analyze file for performance issues"""
    try:
        from app.main import get_performance_profiler
...
@router.post("/api/performance/start-profiling")
...
@router.post("/api/performance/stop-profiling")
...
async def create_workflow(data: dict):
    ...
    try:
        from app.main import get_workflow_automation
...
@router.post("/api/workflow/execute/{workflow_id}")
...
@router.delete("/api/workflow/{workflow_id}")
...
@router.get("/api/workflow/history/{workflow_id}")
