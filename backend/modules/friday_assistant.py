...
class FridayAssistant:
    """FRIDAY - AI-powered code completion and suggestions with advanced features"""
    ...
    def __init__(self):
        ...
        if GEMINI_AVAILABLE:
            try:
                ...
                logger.info(" Gemini AI integration enabled for enhanced FRIDAY features")
        ...
        if not self.ai_provider and OPENROUTER_AVAILABLE:
            try:
                ...
                logger.info(" OpenRouter AI integration enabled as fallback for FRIDAY features")
            except Exception as e:
                logger.warning(f"Failed to initialize OpenRouter: {e}")
        
        if not self.ai_provider:
            logger.warning("  No AI provider available - using pattern-based suggestions only")
...
# Global instance
friday_assistant = None


def get_friday_assistant():
    """Get or create FRIDAY Assistant instance"""
    global friday_assistant
    if friday_assistant is None:
        friday_assistant = FridayAssistant()
    return friday_assistant
