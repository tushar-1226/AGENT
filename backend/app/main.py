...
async def execute_command(command: dict):
    ...
    user_id = command.get("user_id", 1)  # Default to user 1 if not provided
    project_id = command.get("project_id")
    response_text = result.get("response", "Command executed")
    
    try:
        project_manager.add_chat_message(
            message=text,
            response=response_text,
            project_id=project_id,
            user_id=user_id
        )
        logger.info(f"✅ Chat message saved to history (HTTP): {text[:50]}...")
        
        # Broadcast new message notification to all clients
        logger.info(f"📢 Broadcasting new_chat_message to {len(manager.active_connections)} clients")
...
async def upload_document(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Store metadata in a simple JSON file
        metadata = {
            "id": doc_id,
        ...
        logger.info(f"Uploaded document: {file.filename} -> {safe_filename}")
...
async def list_documents():
    ...
    try:
        ...
        documents = []
        for metadata_file in docs_dir.glob("*.json"):
            try:
...
async def summarize_document_endpoint(doc_id: str):
    ...
    try:
        ...
        # Use RAG engine to summarize
        rag = get_rag_engine()
        summary_result = rag.summarize_document(file_path, use_ai=True)
        ...
        summary_result['document'] = metadata
    ...
    try:
        ...
        while True:
            ...
            if message.get("type") == "command":
                ...
                # Save to chat history
                user_id = message.get("user_id", 1)  # Default to user 1 if not provided
                ...
                try:
                    ...
                    logger.info(f"✅ Chat message saved to history: {text[:50]}...")
...
# ============= Development Tools: TODO Tracker =============

todo_tracker_instance = None

def get_todo_tracker():
    """Lazy-load TODO tracker"""
    global todo_tracker_instance
    if todo_tracker_instance is None:
        from modules.todo_tracker import TODOTracker
        todo_tracker_instance = TODOTracker(".")  # Will be set to project path
        logger.info("Initialized TODOTracker")
    return todo_tracker_instance


@app.post("/api/devtools/todos/scan")
async def scan_todos(data: dict):
    ...
    try:
        ...
        logger.error(f"TODO scan error: {e}")
        return JSONResponse(status_code=500, content={"error": str(e)})


@app.get("/api/devtools/todos/stats")
async def get_todo_stats(path: str = "."):
    """Get TODO statistics"""
    try:
        from modules.todo_tracker import TODOTracker
        ...
        logger.error(f"TODO stats error: {e}")
        return JSONResponse(status_code=500, content={"error": str(e)})


@app.get("/api/devtools/todos/filter")
async def filter_todos(path: str = ".", priority: str = None, todo_type: str = None):
    """Filter TODOs by priority or type"""
    try:
        from modules.todo_tracker import TODOTracker
        tracker = TODOTracker(path)
        tracker.scan()
        ...
        if priority:
            todos = tracker.filter_by_priority(priority)
        elif todo_type:
            todos = tracker.filter_by_type(todo_type)
        else:
            todos = tracker.todos
        ...
        logger.error(f"TODO filter error: {e}")
        return JSONResponse(status_code=500, content={"error": str(e)})


@app.get("/api/devtools/todos/search")
async def search_todos(path: str = ".", q: str = ""):
    """Search TODOs by text"""
    try:
        if not q:
            return JSONResponse(status_code=400, content={"error": "Search query required"})
            
        from modules.todo_tracker import TODOTracker
        tracker = TODOTracker(path)
        tracker.scan()
        todos = tracker.search(q)
...
try:
    from modules.advanced_endpoints import router as advanced_router
    app.include_router(advanced_router)
    logger.info("✅ Advanced endpoints router mounted")
except Exception as e:
    logger.error(f"❌ Failed to mount advanced endpoints: {e}")
