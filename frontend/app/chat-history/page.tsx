...
export default function ChatHistoryPage() {
  ...
  // Listen for new message broadcasts via WebSocket
  useEffect(() => {
    console.log('WebSocket message received:', lastMessage);
    if (lastMessage?.type === 'new_chat_message') {
      console.log('📨 New chat message detected via WebSocket, refreshing...');
      fetchChatHistory(true);
    }
  }, [lastMessage, fetchChatHistory]);

  useEffect(() => {
    // Initial fetch
    fetchChatHistory();

    // Set up auto-refresh polling (every 2 seconds) as fallback
    if (autoRefresh) {
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8 flex justify-between items-start">
          ...
          {/* Control buttons */}
          <div className="flex gap-3">            {/* WebSocket Status Indicator */}
            <div 
            ...
            >
              ...
              <span className="text-xs">{wsStatus === 'connected' ? 'Live' : wsStatus === 'connecting' ? 'Connecting' : 'Offline'}</span>
            </div>
                        <button
            ...
            <button
              ...
              title={autoRefresh ? 'Auto-refresh enabled (real-time via WebSocket + polling every 2s)' : 'Auto-refresh disabled'}
            >
              <div className={`w-2 h-2 rounded-full ${autoRefresh ? 'bg-green-400 animate-pulse' : 'bg-gray-500'}`} />
              Auto-refresh {autoRefresh ? '(Real-time)' : ''}
            </button>
          </div>
        </div>
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="text-sm text-gray-500">
                    {new Date(chat.timestamp || chat.created_at || Date.now()).toLocaleString()}
                  </span>
                </div>
