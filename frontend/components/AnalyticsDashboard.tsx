...
export default function AnalyticsDashboard() {
    ...
    const loadAnalytics = async () => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:8000/api/analytics/complete?hours=${timeRange === 'week' ? 168 : timeRange === 'month' ? 720 : 8760}`);
            ...
            if (data.api_calls) {
                ...
                setAnalytics({
                    pomodoroSessions: apiCalls.total_calls || 0,
                    tasksCompleted: apiCalls.status_breakdown?.success || 0,
                    focusMinutes: Math.round((apiCalls.avg_response_time || 0) * 60),
                    productivityScore: Math.round((apiCalls.status_breakdown?.success || 0) / (apiCalls.total_calls || 1) * 100),
                    dailyData: (apiCalls.calls_over_time || []).map((item: any) => ({
                        date: new Date(item.time).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
                        tasks: item.count,
                        pomodoros: Math.floor(item.count * 0.7),
                        focusTime: item.count * 25
                    })),
                    tasksByCategory: (apiCalls.top_endpoints || []).map((item: any, index: number) => ({
                        name: item.endpoint.split('/').pop() || 'Unknown',
                        value: item.count
                    })).slice(0, 5)
                });
            }
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2 items-center">
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${autoRefresh
                    ...
                    <button
                        ...
                        className="px-4 py-2 rounded-lg text-sm font-semibold bg-purple-500 text-white hover:bg-purple-600 transition-all"
                    >
                        Refresh Now
                    </button>
